...
static NumericTablePtr kmeans_compute(size_t rankId, ccl::communicator &comm,
    ...
    NumericTablePtr centroids(new HomogenNumericTable<cpu_algorithmFPType>());

    centroids->deserialize(outArch);

    /* Create an algorithm to compute k-means on local nodes */
    kmeans_cpu::Distributed<step1Local, cpu_algorithmFPType> localAlgorithm(
        nClusters);
    ...
    if (isRoot) {
        ...
        ret_cost = masterAlgorithm.getResult()
                       ...
                       ->getValue<cpu_algorithmFPType>(0, 0);
...
static bool isCenterConverged(const cpu_algorithmFPType *oldCenter,
                              const cpu_algorithmFPType *newCenter, size_t dim,
                              double tolerance) {

    cpu_algorithmFPType sums = 0.0;
...
static bool areAllCentersConverged(const NumericTablePtr &oldCenters,
    ...
    BlockDescriptor<cpu_algorithmFPType> blockOldCenters;
    oldCenters->getBlockOfRows(0, rows, readOnly, blockOldCenters);
    cpu_algorithmFPType *arrayOldCenters = blockOldCenters.getBlockPtr();

    BlockDescriptor<cpu_algorithmFPType> blockNewCenters;
    newCenters->getBlockOfRows(0, rows, readOnly, blockNewCenters);
    cpu_algorithmFPType *arrayNewCenters = blockNewCenters.getBlockPtr();
...
static jlong doKMeansOneAPICompute(
    ...
    const auto kmeans_desc = kmeans_gpu::descriptor<gpu_algorithmFPType>()
