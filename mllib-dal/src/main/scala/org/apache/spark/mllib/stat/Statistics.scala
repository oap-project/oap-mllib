...
object Statistics {
   ...
   * @return [[org.apache.spark.mllib.stat.test.KolmogorovSmirnovTestResult]] object containing
  ...
  /*
   * Convenience function to conduct a one-sample, two-sided Kolmogorov-Smirnov test for
   * probability distribution equality. Currently supports the normal distribution, taking
   * as parameters the mean and standard deviation.
   * (distName = "norm")
   * @param data an `RDD[Double]` containing the sample of data to test
