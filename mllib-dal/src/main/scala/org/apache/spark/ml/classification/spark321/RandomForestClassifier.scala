...
import org.apache.spark.ml.classification.{BinaryClassificationSummary, ClassificationSummary, DecisionTreeClassificationModel, ProbabilisticClassificationModel, ProbabilisticClassifier, TrainingSummary}
...
class RandomForestClassifier @Since("1.4.0") (
  ...
  private def trainRandomForestClassifierDAL(dataset: Dataset[_],
    ...
    val inputData = instances.map {
      case instance => instance.features
    }

    val initStartTime = System.nanoTime()
    val rfDAL = new RandomForestClassifierDALImpl(numClasses,
      getNumTrees,
      1,
      1,
      1,
    ...
    val parentModel = rfDAL.train(inputData)
