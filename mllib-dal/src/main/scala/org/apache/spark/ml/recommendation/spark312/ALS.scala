...
//  case class Rating[@specialized(Int, Long) ID](user: ID, item: ID, rating: Float)
   ...
   */
  def trainMLlib[ID: ClassTag]( // scalastyle:ignore
      ratings: RDD[Rating[ID]],
      rank: Int = 10,
      numUserBlocks: Int = 10,
      numItemBlocks: Int = 10,
    ...
    val userIdAndFactors = userInBlocks
      ...
      .mapPartitions(
        ...
        // Preserve the partitioning because IDs are consistent with the partitioners
        // in userInBlocks and userFactors.
        },
    ...
    ratings
      .mapPartitions { iter =>
        ...
        iter.flatMap { r =>
          ...
          builders.iterator.zipWithIndex.filter(_._1.size > 0).map {
            case (block, idx) =>
  ...
  private def computeFactors[ID](
    ...
    val srcOut = srcOutBlocks.join(srcFactorBlocks).flatMap {
      case (srcBlockId, (srcOutBlock, srcFactors)) =>
        srcOutBlock.iterator.zipWithIndex.map {
