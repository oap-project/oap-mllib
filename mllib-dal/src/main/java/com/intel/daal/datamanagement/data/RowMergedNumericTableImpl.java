 ...
 * @brief Class that provides methods to access a collection of numeric tables as if they are joined
 *        by rows
 */
public class RowMergedNumericTableImpl extends NumericTableImpl {
  ...
  /**
   * Adds the table to the bottom of the row merged numeric table \param table Pointer to the
   * Numeric Table
   */
  public void addNumericTable(NumericTable table) {
    if ((table.getDataLayout().ordinal() & NumericTable.StorageLayout.csrArray.ordinal()) != 0) {
      throw new IllegalArgumentException(
          "can not join numeric table in csr format to row merged numeric table");
    } else {
      ...
      if (ncols == 0) {
        ...
        throw new IllegalArgumentException(
            "incorrect number of columns in the table during joining to a " +
                    "row merged numeric table");
      }
    }
  }
