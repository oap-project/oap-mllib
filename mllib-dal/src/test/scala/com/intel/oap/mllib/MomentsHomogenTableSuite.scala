...
class MomentsHomogenTableSuite extends FunctionsSuite with Logging{
    test("test compute low order moments HomogenTable") {
        ...
        val meanTable = OneDAL.homogenTable1xNToVector(OneDAL.makeHomogenTable(result.meanTable), Common.ComputeDevice.HOST)
        val varianceTable = OneDAL.homogenTable1xNToVector(OneDAL.makeHomogenTable(result.varianceTable), Common.ComputeDevice.HOST)
        val minimumTable = OneDAL.homogenTable1xNToVector(OneDAL.makeHomogenTable(result.minimumTable), Common.ComputeDevice.HOST)
        val maximumTable = OneDAL.homogenTable1xNToVector(OneDAL.makeHomogenTable(result.maximumTable), Common.ComputeDevice.HOST)
