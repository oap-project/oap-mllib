...
class CorrelationHomogenTableSuite extends FunctionsSuite with Logging {
    ...
    test("test compute Correlation HomogenTable") {
        ...
        correlationDAL.cCorrelationTrainDAL(dataTable.getcObejct(), sourceData.length, sourceData(0).length, 1, 1, CommonJob.ComputeDevice.HOST.ordinal(), gpuIndices, result);
